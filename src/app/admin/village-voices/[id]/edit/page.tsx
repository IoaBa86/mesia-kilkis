// src/app/admin/village-voices/[id]/edit/page.tsx
"use client"
...
export default function EditVillageVoicePage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/village-voices/${id}`)
      .then(res => res.json())
      .then(data => {
        if (data.voice) {
          setFormData({
            type: data.voice.type,
            authorName: data.voice.authorName,
            title: data.voice.title,
            content: data.voice.content,
            imageUrl: data.voice.imageUrl || "",
            isActive: data.voice.isActive,
          })
        } else {
          setError('Η μαρτυρία δεν βρέθηκε')
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/village-voices/${id}`, {
        method: 'PATCH',
        ...
        <div className="max-w-4xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Επεξεργασία Μαρτυρίας</h1>
